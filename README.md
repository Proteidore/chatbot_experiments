...
with the file *requirements.txt*
...
It is important because the dotenv module enables a notebook to read key-value pairs from the *.env* file and to set them in its environment. Below, there are links to create one's own keys.
...
Once created, put the keys into the *.env* file 
